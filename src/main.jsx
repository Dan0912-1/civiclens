...
// Non-critical third parties — error monitoring (Sentry) and analytics
// (GA4 + PostHog) — are deferred to the browser's first idle window AFTER paint
// instead of firing during entry-chunk evaluation. On throttled mobile, gtag.js
// (~160 KB) and the Sentry chunk (~150 KB) were fetching inside the boot window
// and starving the app's own JS for bandwidth, pushing LCP past 8s. Loading
// them on idle keeps the critical path lean. Errors/events in the brief
// pre-init window are dropped — the same trade Sentry and PostHog already made
// to be lazy chunks. requestIdleCallback yields until the initial render is
// done; the setTimeout fallback covers Safari < 16.4 and the native webview.
function onIdle(fn) {
...
onIdle(() => {
  // Sentry — async chunk, never pulled into the entry bundle. ErrorBoundary's
  // captureException imports the SDK on demand too, so render errors thrown
  // before this init still report.
