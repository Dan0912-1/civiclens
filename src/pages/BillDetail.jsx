  ...
  return (
    <main className={styles.page}>
      <div className={styles.container}>
        ...
        {/* One canonical place to read the source text. Read-the-bill actions
            generated by personalization are filtered above so students do not
            see two controls with the same destination. */}
        <section className={styles.billTextSection} aria-label="Full bill text">
          <button
            className={styles.billTextToggle}
            onClick={handleOpenFullText}
            disabled={fullTextLoading}
            aria-expanded={fullTextOpen}
            aria-controls="bill-text-reader"
          >
            <span className={styles.billTextToggleTitle}>
              <span className={styles.billTextEyebrow}>Source document</span>
              <span className={styles.billTextHeading}>Full bill text</span>
            </span>
            <span className={styles.billTextToggleAction}>
              {fullTextLoading ? 'Loading…' : fullTextOpen ? 'Hide text ↑' : 'Read text ↓'}
            </span>
          </button>

          {fullTextOpen && (
            <div id="bill-text-reader" className={styles.billTextPanel} aria-live="polite">
              {fullText ? (
                <>
                  <div className={styles.billTextDocumentMeta}>
                    <span>
                  ...
                  {billTextHasDeletions && (
                    <p className={styles.billTextChangeNote}>
                      Struck text was removed in the latest reported version.
                    </p>
                  )}
                  <article className={styles.billTextBody}>
                    {fullTextBlocks.map((block, index) => {
                ...
                <div className={styles.billTextUnavailable}>
                  <p>Full text isn't available in CapitolKey for this bill yet.</p>
                  <a href="#" onClick={e => { e.preventDefault(); openInAppBrowser(billTextUrl) }}>
